...
(function (global) {
    ...
    function processStyleSheets() {
        ...
        const parseCSSRule = rule => {
            /* CUSTOM: Modified this line to includ support for --grid-lanes-polyfill: 1*/
            /* TODO: Apply this to selecting in stylesheet and raw style*/
            if (rule.cssText && (/display\s*:\s*grid-lanes/i.test(rule.cssText) || /--grid-lanes-polyfill:\s*1/i.test(rule.cssText))) {
            ...
            if ("cssRules" in node){
                try {
                    ...
                    for (const rule of rules) {
                        if (rule instanceof CSSImportRule){ // handle import
                            handleRules(rule);
                        } else {
                            parseCSSRule(rule);
                        }
